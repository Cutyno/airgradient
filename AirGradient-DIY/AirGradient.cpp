...
void AirGradient::CO2_Init(int rx_pin, int tx_pin, int baudRate)
{
  ...
  _S8_serial = new SoftwareSerial(rx_pin, tx_pin);
  _S8_serial->begin(baudRate);
  co2 = new S8_UART(*_S8_serial);

  // Setting ABC period
  co2->set_ABC_period(48);
  ...
  if(_debugMsg)
  {
    ...
    Serial.println("");
    // Setting ABC period
    Serial.println("Setting ABC period set to 48 hours...");
    CO2sensor.abc_period = co2->get_ABC_period();
    if (CO2sensor.abc_period == 48) {
